...
export interface ApiContainer extends AiContainer {
  ...
  passwordCredentialRepo: PasswordCredentialRepository;
...
export function createApiContainerForSpace(db: DB, spaceId: string): ApiContainer {
  ...
  return { ...ai, sessionRepo, passwordCredentialRepo, emailTokenRepo, allowedUserRepo, userTrialRepo, trialService, spaceService };
}
